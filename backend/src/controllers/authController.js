...
const { User, Role, Profile, Audit } = require('../models');
...
const authController = {
  ...
  async login(req, res) {
    try {
      ...
      await logAuditEvent(user.id, 'Usuario inició sesión');
  ...
  async logout(req, res) {
    try {
      // Registrar evento de auditoría si hay usuario autenticado
      if (req.user && req.user.id) {
        await logAuditEvent(req.user.id, 'Usuario cerró sesión');
      }
