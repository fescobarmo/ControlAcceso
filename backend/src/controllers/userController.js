...
const userController = {
  ...
  async createUser(req, res) {
    try {
      ...
      await logAuditEvent(req.user?.id, `Usuario creó nuevo usuario: ${username}`);
  ...
  async updateUser(req, res) {
    try {
      ...
      // Registrar evento de auditoría
      await logAuditEvent(req.user?.id, `Usuario actualizó usuario: ${updatedUser.username}`);
  ...
  async deleteUser(req, res) {
    try {
      ...
      // Registrar evento de auditoría
      await logAuditEvent(req.user?.id, `Usuario eliminó usuario: ${user.username}`);
